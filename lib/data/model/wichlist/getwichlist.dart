class getWichListResponse {
  String? status;
  int? count;
  List<WichListData>? data;

  getWichListResponse({this.status, this.count, this.data});

  getWichListResponse.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    count = json['count'];
    if (json['data'] != null) {
      data = <WichListData>[];
      json['data'].forEach((v) {
        data!.add(new WichListData.fromJson(v));
      });
    }
  }
...
class WichListData {
  ...
  WichListData.fromJson(Map<String, dynamic> json) {
    sold = json['sold'];
